...
# `useMemo`
`useMemo` - это еще один хук в React, который позволяет мемоизировать результат выполнения функции, чтобы избежать лишних вычислений при повторных рендерингах компонента. Он используется в тех случаях, когда вы хотите кэшировать результат вычисления, чтобы он не вычислялся заново при каждом рендеринге, если его зависимости не изменились.

`useMemo` принимает два аргумента: **функцию** и **массив зависимостей**. Функция - это функция, результат выполнения которой будет мемоизирован. Массив зависимостей содержит переменные, от которых будет зависеть результат функции.
...
const Sum: React.FC<Props> = ({ a, b }) => {
  // Используем useMemo, чтобы мемоизировать результат сложения
  const sum = useMemo(() => {
    console.log('Вычисление суммы'); // Будет вызвано только при изменении a или b
    return a + b;
  }, [a, b]);
...
# `React.memo`
`React.memo` - это функция высшего порядка (**Higher Order Function**) в библиотеке React, которая позволяет мемоизировать компоненты и предотвращать их повторное рендеринг при отсутствии изменений в их пропсах. Она подобна useMemo, но работает с целыми компонентами, а не только с отдельными значениями.
```typescript
interface Props {
  a: number;
  b: number;
...
const Sum: React.FC<Props> = ({ a, b }) => {
  console.log('Рендер компонента Sum'); // Будет вызвано при каждом рендеринге
  return <div>Сумма: {a + b}</div>;
};

// Оборачиваем компонент Sum в React.memo
const MemoizedSum = React.memo(Sum);
