...
#### `new` - это ключевое слово для создания **экземпляра класса**. При использовании **
new** на выходе мы получаем объект, который являтся экзмепляром класса. Когда мы вызываем **
